...
MODEL_ALIASES = {
    "small": "autogluon/moirai-1.0-r-small",
    "base": "autogluon/moirai-1.0-r-base",
    "large": "autogluon/moirai-1.0-r-large",
}


class MoiraiModel(AbstractTimeSeriesModel):
    """Chronos pretrained time series forecasting models, based on the original
    `Moirai model <https://github.com/SalesforceAIResearch/uni2ts>`_ implementation.

    Chronos is family of pretrained models, based on the T5 family, with number of parameters ranging between 8M and 710M.
    The full collection of Moirai models is available on
    `Hugging Face <https://huggingface.co/collections/Salesforce/moirai-10-r-models-65c8d3a94c51428c300e0742>`_. For Moirai small,
    base, and large variants a GPU is required to perform inference efficiently.

    Uni2TS is a PyTorch based library for research and applications related to Time Series Transformers. This library aims to 
    provide a unified solution to large-scale pre-training of Universal Time Series Transformers. 
    Uni2TS also provides tools for fine-tuning, inference, and evaluation for time series forecasting.
    See [Woo2024]_ for more information.

    References
    ----------
    .. [Woo2024] Woo, G., Liu, C., Kumar, A., Xiong, C., Savarese, S., & Sahoo, D.  
       Unified Training of Universal Time Series Forecasting Transformers [Conference paper]. 
    ...
    Other Parameters
    ----------------
    model_path: str, default = "autogluon/moirai-1.0-r-small"
        ...
        Moirai models (i.e., ``autogluon/moirai-1.0-r-{model_size}``) can be specified with aliases
    ...
    torch_dtype : torch.dtype or {"auto", "bfloat16", "float32", "float64"}, default = "auto"
        ...
        original Moirai models are specified and the model size is ``small``, ``base``, or ``large``, the
    ...
    default_model_path = "autogluon/moirai-1.0-r-small"
    ...
    def __init__(
        ...
        **kwargs,  # noqa
        ...
        name = (name if name is not None else "Moirai") + f"[{model_path_safe}]"
        ...
        self.model_pipeline: None #Optional[Any] = None  # of type OptimizedMoiraiPipeline
    ...
    @classmethod
    def load(cls, path: str, reset_paths: bool = True, verbose: bool = True) -> "MoiraiModel":
    ...
    def ag_default_config(self) -> Dict[str, Any]:
        ...
        defined in MODEL_CONFIGS. For now, these are ``autogluon/moirai-1.0-r-*`` family of models.
        """
